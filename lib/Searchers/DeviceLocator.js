...
class DeviceLocator {
	constructor(pubSub, md5, timer, fetch, deviceFactory, activeSearcher, passiveSearcher, accessPointSearcher, storageService, serviceExecutor) {
		this._pubSub = pubSub;
	...
	_removeLostDevice(deviceUUID) {
		for (var i = 0; i < this.devices.length; i++) {
			if (this.devices[i].uuid === deviceUUID) {
				...
				this._pubSub.pub("deviceLocator.lost", lostDevice);
				i--;
			}
		}
	...
	_deviceFound(headers, ignoreDebounce) {
		...
		this._deviceFactory.create(headers.location, headers, previouslyFound, 0).then(device => {
			...
			this._pubSub.pub('deviceLocator.found', device, isNew);
		});
	}
