...
class SSDPClient {
    constructor(eventPrefix, pubSub, utilities, udpSocket, networkingUtils) {
    ...
    onStopListening(socket, status) { // nsIUDPSocketListener
        this._pubSub.pub(this._eventPrefix + 'ssdpClient.close', status);
    }
    onPacketReceived(socket, message) { // See: https://bugzilla.mozilla.org/show_bug.cgi?id=952927
        ...
        this._pubSub.pub(this._eventPrefix + 'ssdpClient.messageReceived', headers);
    }
