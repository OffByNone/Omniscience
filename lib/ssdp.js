'use strict';

const { Cc, Ci } = require('chrome'); // https://addons.mozilla.org/en-US/developers/docs/sdk/latest/dev-guide/tutorials/chrome.html
const { Class } = require('sdk/core/heritage'); // https://addons.mozilla.org/en-US/developers/docs/sdk/latest/modules/sdk/core/heritage.html
const { emit } = require('sdk/event/core'); // https://addons.mozilla.org/en-US/developers/docs/sdk/latest/modules/sdk/event/core.html
const { merge } = require("sdk/util/object"); // https://developer.mozilla.org/en-US/Add-ons/SDK/Low-Level_APIs/util_object
const { EventTarget } = require("sdk/event/target"); // https://developer.mozilla.org/en-US/Add-ons/SDK/Low-Level_APIs/event_target
const { Buffer } = require('sdk/io/buffer');

const SSDP = Class({
    extends: EventTarget,
    initialize: function initialize(options) {
        EventTarget.prototype.initialize.call(this, options);
        merge(this, options);
        
        this.socket = Cc['@mozilla.org/network/udp-socket;1'].createInstance(Ci.nsIUDPSocket); // http://dxr.mozilla.org/mozilla-central/source/netwerk/base/public/nsIUDPSocket.idl
        this.multicastIP = '239.255.255.250';
        this.multicastPort = 1900;
        this.socket.init(this.sourcePort, false);
        this.socket.asyncListen(this);
        
        this.on('message', (message, rinfo) => {
            var headers = this._parseHeaders(message.data);
            emit(this,'deviceresponse', headers);
        });
    },
    search : function search (service) {
        var message = new Buffer(
            'M-SEARCH * HTTP/1.1\r\n' +
            'HOST: ' + this.multicastIP + ':' + this.multicastPort + '\r\n' +
            'ST: ' + service + '\r\n' +
            'MAN: "ssdp:discover"\r\n' +
            'MX: 1\r\n\r\n');
        
        this.socket.send(this.multicastIP, this.multicastPort, message, message.length);
    },
    shutdown : function close (){
        this.socket.close();
    },
    startListening : function(){
        this.socket.joinMulticast(this.multicastIP);
    },
    stopListening : function(){
        this.socket.leaveMulticase(this.multicastIP);
    },
    onStopListening: function onStopListening(_, aStatus) { // nsIUDPSocketListener
        emit(this, 'close', aStatus);
    },
    onPacketReceived: function onPacketReceived(_, aMessage) { // See: https://bugzilla.mozilla.org/show_bug.cgi?id=952927
        let buffer = new Buffer(aMessage.data);
        let message = {data: aMessage.data, rawData: buffer};
        emit(this, 'message', message, aMessage);
    },
    _parseHeaders : function _parseHeaders(headerString) {
        var headers = {};
        headerString.split("\r\n").forEach(x => {
            if(x === null || x.indexOf(":") === -1) return;
            var colon = x.indexOf(":");
            headers[x.substring(0,colon).toUpperCase()] = x.substring(colon+1).trim();
        });
        return headers;
    }
});

const createSSDP = (options) => new SSDP(options);
exports.createSSDP = createSSDP;
exports = SSDP;