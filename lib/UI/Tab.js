		...
		this._tabs.open({
			...
			onLoad: (tab) => {
				...
				this._pageWorker.port.on('CallService', (uniqueId, service, serviceMethod, data) =>
					this._deviceServices.callService(service.controlUrl, service.responseHash, service.type.urn, serviceMethod, data).then( (data) => this.emit(serviceMethod + "Response" + uniqueId, data) ));
