...
const Tab = Class({
    ...
    initialize: function initialize(filePicker, deviceServices, emitter) {
        ...
        this._deviceServices.on( 'SomethingHappened', ( device, request ) => {
        	console.log(device);
        	console.log(request);
        	this.emit( 'SomethingHappened', device, request );
        });
    },
    emit: function emit(event, data){
        ...
        data = data == null ? null : JSON.parse(JSON.stringify(data, replacer));
