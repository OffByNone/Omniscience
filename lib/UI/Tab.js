		...
		this._tabs.open({
			...
			onLoad: (tab) => {
				...
				this._deviceServices.on( 'deviceFound', ( device ) => {
					this.emit( 'additionalInformationFound', device, event, request );
				});
				this._pageWorker.port.on('ServiceCall', (service, serviceMethod, data) =>
					this._deviceServices.callService(service, serviceMethod, data).then( (data) => this.emit(serviceMethod + "response", data) ));
