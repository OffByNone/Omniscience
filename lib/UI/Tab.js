	...
	openFocus: function openFocus() {
		...
		this._tabs.open({
			...
			onLoad: (tab) => {
				...
				this._pageWorker.port.on('shareFile', (uniqueId, deviceService, file) => {
					var result = this._fileService.shareFile(deviceService, file);
					this.emit('emitResponse', uniqueId, result);
				});
				this._pageWorker.port.on('CallService', (uniqueId, service, serviceMethod, data) =>
					this._deviceServices.callService(service.controlUrl, service.type.raw, serviceMethod, data).
						then( (response) => this.emit("CallServiceResponse", uniqueId, response) ));

				this._pageWorker.port.on('Subscribe', (uniqueId, subscriptionUrl, subscriptionId, serviceHash, timeout) =>
					this._deviceServices.subscribe(subscriptionUrl, subscriptionId, serviceHash, timeout).
						then( (eventSubscriptionId) => this.emit("SubscribeResponse", uniqueId, eventSubscriptionId) ));
				this._pageWorker.port.on('Unsubscribe', (uniqueId, subscriptionUrl, subscriptionId, serviceHash) =>
					this._deviceServices.unsubscribe(subscriptionUrl, subscriptionId, serviceHash).
						then( () => this.emit("UnsubscribeResponse", uniqueId) ));
