...
const createDevice = function (location, type) {
	...
	Request({
		...
		onComplete: (response) => {
			...
			device.setProperty('presentationUrl', responseObj.device.presentationurl);
			device.setProperty('address', new URL(base));
			device.setProperty('type', responseObj.device.devicetype);
			device.setProperty('name', responseObj.device.friendlyname);
			device.setProperty('manufacturer', responseObj.device.manufacturer, 'name');
			device.setProperty('manufacturer', responseObj.device.manufacturerurl, 'url');
			device.setProperty('model', responseObj.device.modelnumber, 'number');
			device.setProperty('model', responseObj.device.modeldescription, 'description');
			device.setProperty('model', responseObj.device.modelname, 'name');
			device.setProperty('model', responseObj.device.modelurl, 'url');
			device.setProperty('udn', responseObj.device.udn);
			device.setProperty('version', responseObj.specversion.major + '.' + responseObj.specversion.minor);
			device.setProperty('serialNumber', responseObj.device.serialnumber);
...
const Device = Class({
	...
	initialize: function initialize(rawResponse) {
		EventTarget.prototype.initialize.call(this, null);
	...
	setProperty: function setProperty(property, value, subProperty) {
		if (!value) return;
		if (!subProperty) {
			this[property] = value;
			return;
		...
		this[property][subProperty] = value;
	}
});
