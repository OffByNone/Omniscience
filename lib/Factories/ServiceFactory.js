...
const ServiceFactory = Class({
	...
	create: function create(serviceInfo){
		var service = {};
		serviceInfo.methods.forEach(method => this._addMethod(service, method, serviceInfo.type.urn));
	...
	_addMethod: function (service, serviceInfoMethod, urn){
		...
		service[serviceInfoMethod.name] = function(serviceId, controlUrl, params){
	...
	_validateParam: function (parameter, params){
		...
		if(!params.hasOwnProperty(parameter.name)) throw `Missing required argument: ${parameter.name}`;
		...
		if(parameter.allowedValues.length > 0 && parameter.allowedValues.every(allowedValue => allowedValue != param))
			throw `value for argument ${parameter.name} is not allowed`;
		if(parameter.allowedValueRange.maximum != null && parameter.allowedValueRange.minimum != null && parameter.allowedValueRange.step != null){
			...
			if(isNaN(paramNum)
				...
				throw `argument ${parameter.name} is invalid. It must be a number between ${parameter.allowedValueRange.minimum} and ${parameter.allowedValueRange.maximum}, and incremented by ${parameter.allowedValueRange.step}.`;
		}
	}
});
