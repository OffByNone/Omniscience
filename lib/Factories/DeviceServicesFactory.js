...
//todo: remove this and replace with ECMA6 fetch
...
const DeviceServicesFactory = Class({
    ...
    createMatchStickService: function createMatchStickService(){
        return new MatchStickService(Emitter, new MessageFactory(), this._messageServiceFactory.createMessageService(), this._defer);
    },
    createChromecastService: function createChromecastService(){
        return new ChromecastService(Emitter, new MessageFactory(), this._defer);
    },
    createFirestickService: function createFirestickService(){
        return new FirestickService(Emitter, new MessageFactory(), this._defer);
    },
    createMediaRendererService: function createMediaRendererService(){
        return new MediaRendererService(Emitter, this.createAVTransportService(), this.createConnectionManagerService(), this.createRenderingControlService(), this._defer);
    },
    createGenericDeviceService: function createGenericDeviceService(){
        return new GenericDeviceService( Emitter, new MessageFactory(), this._defer );
    },
    createAVTransportService: function createAVTransportService(){
        return new AVTransportService( Emitter, new SOAPService( Request, this._defer, this._componentFactory.createDOMParser(), utilities), new SubscriptionService( xhr, this._defer ), this._defer );
    },
    createConnectionManagerService: function createConnectionManagerService(){
        return new ConnectionManagerService( Emitter, new SOAPService( Request, this._defer, this._componentFactory.createDOMParser(), utilities), new SubscriptionService( xhr, this._defer ), this._defer );
    },
    createRenderingControlService: function createRenderingControlService(){
        return new RenderingControlService( Emitter, new SOAPService( Request, this._defer, this._componentFactory.createDOMParser(), utilities), new SubscriptionService( xhr, this._defer ), this._defer );
    }
});
