...
const DeviceServicesFactory = Class({
    ...
    createAVTransportService: function createAVTransportService(){
        return new AVTransportService( Emitter, new SOAPService( Request, defer, JXON, this._componentFactory.createDOMParser(), utilities), new SubscriptionService( xhr, defer ) );
    },
    createConnectionManagerService: function createConnectionManagerService(){
        return new ConnectionManagerService( Emitter, new SOAPService( Request, defer, JXON, this._componentFactory.createDOMParser(), utilities), new SubscriptionService( xhr, defer ) );
    },
    createRenderingControlService: function createRenderingControlService(){
        return new RenderingControlService( Emitter, new SOAPService( Request, defer, JXON, this._componentFactory.createDOMParser(), utilities), new SubscriptionService( xhr, defer ) );
    }
});
