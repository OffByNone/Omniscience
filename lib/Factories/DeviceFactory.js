...
const { AVTransportDevice } = require('../Entities/Devices/AVTransportDevice');
	...
	createDevice: function createDevice(location, headers) {
        ...
        this._request({
            ...
            onComplete: (response) => {
                ...
                else if(responseObj.device.servicelist.service.servicetype === constants.ServiceTypes.filter(y=> y[0] === "AV Transport").map(y=> y[1])[0] 
                        || (typeof responseObj.device.servicelist.service.some === "function" && responseObj.device.servicelist.service.some(x=> x.servicetype === constants.ServiceTypes.filter(y=> y[0] === "AV Transport").map(y=> y[1])[0])))
                    device = this._createAVTransport();                
                else
                ...
                if (!responseObj.device.iconlist || !responseObj.device.iconlist.icon)
                    ...
                    device.icons.push({
                        ...
                        url: this._toURL(base, responseObj.device.iconlist.icon.url, location)
                    });
                }
                if (!responseObj.device.servicelist || !responseObj.device.servicelist.service)
                    ...
                    device.services.push({
                        controlUrl: this._toURL(base, responseObj.device.servicelist.service.controlurl, location),
                        eventSubUrl: this._toURL(base, responseObj.device.servicelist.service.eventsuburl, location),
                        scpdUrl: this._toURL(base, responseObj.device.servicelist.service.scpdurl, location),
