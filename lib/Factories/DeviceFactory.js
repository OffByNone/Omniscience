...
const DeviceFactory = Class({
	extends: EventTarget,
	initialize: function initialize(domParser, request, defer, messageFactory, deviceServices, emitter, xmlParser, utilities, md5, serviceFactory) {
				...
				if(!device || responseHash !== device.responseHash){
					...
					if(deviceXml){
						...
						servicesXml.forEach(serviceXml => {
							var service = this._serviceFactory.create(serviceXml, base, location);
					...
					device.id = this._md5.md5(location);
