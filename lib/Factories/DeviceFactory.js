...
class DeviceFactory {
    ...
    create(location, headers, device, attempt) {
        ...
        return this._fetch(location).then( (response) => {
            ...
            if (!device || (responseHash !== device.responseHash && device.fromAddress === headers.fromAddress)) {
                ...
                device = this.parseDeviceXml(deviceXml, base, location, headers.ServerIP);
    ...
    parseDeviceXml(deviceXml, base, location, serverIP) {
    	...
    	childDevices.forEach((childDeviceXml) => device.childDevices.push(this.parseDeviceXml(childDeviceXml, base, location, serverIP)));
