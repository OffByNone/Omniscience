...
const { MatchStick } = require('../Devices/MatchStick');
...
const DeviceFactory = Class({
	...
	createDevice: function createDevice(location, headers) {
        ...
        Request({
            ...
            onComplete: (response) => {
                ...
                if(responseObj.device.modelname === constants.MatchstickModelName)
                    device = this._createMatchStick();
                else if(responseObj.device.modelname === constants.ChromecastModelName)
                ...
                if (device.model.name === constants.MatchstickModelName) device.getAdditionalInformation();
    ...
    _createMatchStick: function createMatchStick(){
        return new MatchStick(Emitter, this._messageFactory, this._messageFactory.createMessenger());
    },
    _createChromecast: function createChromecast(){
        return new Chromecast(Emitter);
    },
    _createGenericDevice: function createGenericDevice(){
        return new GenericDevice(Emitter);
    }
});
