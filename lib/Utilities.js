...
function getAddressForDevice(device) {
	var deviceIp = new URL(device.address).host;
	//todo: pains me to hardcode http
	var myIp = "http://" + getMyIPAddresses().filter(x=> areIPsInSameSubnet(x, deviceIp))[0];
...
function getControlUrl(device, serviceType){
    return device.services.filter(x=> x.type.urn === serviceType)[0].controlUrl;
}

function getEventSubUrl(device, serviceType){
    return device.services.filter(x=> x.type.urn === serviceType)[0].eventSubUrl;
}

function toURL(baseUrl, path, location){
	//todo: fix this method.  There are tons of problems in here
	//todo: url accepts base as a second argument use that instead of trying to do stuff
	//todo: isValidUri(uri)
