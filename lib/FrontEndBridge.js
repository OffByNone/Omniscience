...
class FrontEndBridge extends Eventable {
	...
	handleMessageFromFrontEnd(eventType, uniqueId, ...data) {
		switch (eventType) {
			case 'Subscribe':
				let [eventSubUrl, serviceHash, serverIP, timeoutInSeconds, subscriptionId] = data;

				let directResponsesTo = `http://${serverIP}:${this._simpleServer.port}/events/${serviceHash}`;
				this._simpleServer.registerPath(`/events/${serviceHash}`, (request) => this.sendToFrontEnd("UPnPEvent", serviceHash, request.body));
