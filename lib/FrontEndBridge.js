...
class FrontEndBridge extends Eventable {
	constructor(subscriptionService, serviceExecutor, fileUtilities, fileSharer, deviceService, httpServer, tcpConversationalist) {
		super();
	...
	handleMessageFromFrontEnd(eventType, uniqueId, ...data) {
		switch (eventType) {
			case 'Subscribe':
				...
				this._httpServer.registerPath(directResponsesTo, (request) => this.sendToFrontEnd("UPnPEvent", serviceUUID, request.body));
			...
			case 'search':
				...
				break;
			case 'sendTCP':
				this._tcpConversationalist.send(...data).then((response) => this.sendToFrontEnd("emitResponse", uniqueId, response));
				break;
		}
	}
