...
const SOAPService = Class({
    ...
    post: function post(url, serviceName, methodName, parameters){
        ...
        this._buildRequest(url, serviceName, methodName, parameters, deferred).post();
        return deferred.promise;
    },
    _buildRequest: function _buildRequest(url, serviceName, methodName, parameters, deferred){
        var parametersXML = this._parametersToXML(parameters);
        return this._request({
            url: url,
            headers: { SOAPAction: `"${serviceName}#${methodName}"` },
            contentType: Constants.SOAP.ContentType,
            content: this._utilities.format(Constants.SOAP.Body, serviceName, methodName, parametersXML),
            onComplete: (response) => {
