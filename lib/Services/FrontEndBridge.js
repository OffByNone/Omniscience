...
class FrontEndBridge extends Eventable {
	...
	handleMessageFromFrontEnd(eventType, uniqueId, ...data) {
		switch (eventType) {
			case 'Subscribe':
				var [eventSubUrl, subscriptionId, serviceHash, serverIP, timeoutInSeconds] = data;
				...
				this._httpServer.registerPath(directResponsesTo, (request) => {
					this.sendToFrontEnd("UPnPEvent", serviceHash, request.body);
					//this._httpResponder.sendOkResponse(request.socket);
				});
				this._subscriptionService.subscribe(directResponsesTo, eventSubUrl, subscriptionId, timeoutInSeconds).then((subscriptionId) => this.sendToFrontEnd("emitResponse", uniqueId, subscriptionId));
				break;
			case 'Unsubscribe':
			...
			case 'CallService':
				...
				this._serviceExecutor.callService(service.controlUrl, service.hash, method, info).then((response) => this.sendToFrontEnd("emitResponse", uniqueId, response));
				break;
			case 'chooseFiles':
