...
const MediaRendererService = Class({
	...
	initialize: function initialize(emitter, defer, avTransportService, connectionManagerService, renderingControlService) {
        ...
        this._avTransport.on('SomethingHappened', ( device, request ) => {
        	this._emitter.emit(this, 'SomethingHappened', device, request );
        });
        this._connectionManager = connectionManagerService;
        this._connectionManager.on('SomethingHappened', ( device, request ) => {
        	this._emitter.emit(this, 'SomethingHappened', device, request );
        });
        this._renderingControl = renderingControlService;
        this._renderingControl.on('SomethingHappened', ( device, request ) => {
        	this._emitter.emit(this, 'SomethingHappened', device, request );
        });
        this._defer = defer;
	},
