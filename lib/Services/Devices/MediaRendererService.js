...
const MediaRendererService = Class({
	...
	getAdditionalInformation: function getAdditionalInformation(device) {
		...
		this._connectionManager.subscribe(device);
		this._renderingControl.subscribe(device);

		this._renderingControl.getMute(device).then( response => {
			device.isMuted = response.isMuted;
			this._emitter.emit(this, "additionalInformationFound", device);
		});

		this._renderingControl.getVolume(device).then( response => {
			device.volume = response.volume;
			this._emitter.emit(this, "additionalInformationFound", device);
		});
    ...
    setMute: function toggleMute(device, desiredMute){
        var deferred = this._defer();
        this._renderingControl.setMute(device, desiredMute).then(response => deferred.resolve(response));
    ...
    pause: function pause(device){
        var deferred = this._defer();
        this._avTransport.pause(device).then(response => deferred.resolve(response));
