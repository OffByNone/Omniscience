...
const MediaRendererService = Class({
	...
	getAdditionalInformation: function getAdditionalInformation(device) {
	   this._connectionManager.getProtocolInfo(device).then( response => {
           var parsedResponse = response["s:envelope"]["s:body"]["u:getprotocolinforesponse"].sink.split(',');
           if(parsedResponse.some(x => x.indexOf(":video/") >= 0)) device.videoCapable = true;
           if(parsedResponse.some(x => x.indexOf(":audio/") >= 0)) device.audioCapable = true;
           if(parsedResponse.some(x => x.indexOf(":image/") >= 0)) device.imageCapable = true;
