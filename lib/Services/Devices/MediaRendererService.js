...
const MediaRendererService = Class({
	...
	getAdditionalInformation: function getAdditionalInformation(device) {
	   this._connectionManager.getProtocolInfo(device).then( response => {
           if(response.some(x => x.contentFormat.medium.indexOf("video") === 0)) device.videoCapable = true;
           if(response.some(x => x.contentFormat.medium.indexOf("audio") === 0)) device.audioCapable = true;
           if(response.some(x => x.contentFormat.medium.indexOf("image") === 0)) device.imageCapable = true;
    ...
    launchMedia: function launchMedia(device, serverAddress, fileUri){
        this.loadMedia(device, serverAddress, fileUri).then( ( response ) => this.play(device));
    },
