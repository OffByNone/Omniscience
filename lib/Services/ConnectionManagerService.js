...
const ConnectionManagerService = Class({
	extends: EventTarget,
	initialize: function initialize(emitter, defer, soapService, subscriptionService) {
		this._defer = defer;
		...
		this._subscriptionService.on( 'SomethingHappened', ( device, request ) => {
			this._emitter.emit( this, 'SomethingHappened', device, request );
		});
	},
	getCurrentConnectionIds: function getCurrentConnectionIds(device){
		...
		this._soapService.post(Utilities.getControlUrl(device, this._serviceType),
                               this._serviceType, "GetCurrentConnectionIds"
                              ).then(response => {
	...
	getCurrentConnectionInfo: function getCurrentConnectionInfo(device){
		var deferred = this._defer();
		this._soapService.post(Utilities.getControlUrl(device, this._serviceType),
                               this._serviceType, "GetCurrentConnectionInfo", { ConnectionID: device.connectionId }
                              ).then(response => {
                              	//todo: parse xml respose here;
                              	//todo: call from getAdditionalInfo
                              	deferred.resolve(response);
                              });
		return deferred.promise;
	...
	subscribe: function subscribe(device){
		var deferred = this._defer();
		this._subscriptionService.subscribe(device, this._serviceType, 'ConnectionManager' ).then( response => deferred.resolve( response ) );
		return deferred.promise;
	},
