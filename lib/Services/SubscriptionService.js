class SubscriptionService {
	constructor(fetch, eventer, httpServer, httpResponder) {
		this._fetch = fetch;
	...
	subscribe(subscriptionUrl, eventSubscriptionId, serviceHash, serverIP, timeout) {
        ...
        this._httpServer.registerPath(directResponsesTo, (request, resolve) => {
        	this._eventer.emit("subscriptionService.UPnPEvent", serviceHash, request.body);
			...
			//todo: this function probably doesn't belong in here
    			...
    			if (!response.ok) {
    				if(response.status == 412){
    					...
    					return this.subscribe(subscriptionUrl, subscriptionId, serviceHash, serverIP, timeout);
    				}
    				else
