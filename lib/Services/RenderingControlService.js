...
const RenderingControlService = Class({
	...
	getMute: function getMute(device){
        ...
        this._soapService.post(Utilities.getControlUrl(device, this._serviceType),
                               this._serviceType, "GetMute", { InstanceID: device.instanceId, Channel: 'Master' }
                              			...
                              			var isMuted = response.querySelector("Envelope Body GetMuteResponse CurrentMute").innerHTML == 1;
	...
	getVolume: function getVolume(device){
        ...
        this._soapService.post(Utilities.getControlUrl(device, this._serviceType),
                                      this._serviceType, "GetVolume", { InstanceID: device.instanceId, Channel: 'Master' }
                                     	...
                                     	var volume = response.querySelector("Envelope Body GetVolumeResponse CurrentVolume").innerHTML;
    ...
    _parseEventRequest: function _parseEventRequest(request){
    	if(!request.body) return null;//my receiver thinks it is sending a body as it sends content lengths but there is no body
    	...
    	Array.prototype.slice.call(lastChanges).map((lastChange)=>{
    		var eventString = lastChange.innerHTML.replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&quot;/g,'"'); //I can only test against my xbox but it is escaping all the xml in here, doesnt seem like it should but it does
