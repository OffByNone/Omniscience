...
const DeviceLocatorService = Class({
    ...
    initialize: function initialize(utilities, emitter, timers, ssdpFactory, deviceFactory) {
        ...
        this._deviceFactory.on("additionalInfoFound", device => this._addUpdateDevice(device) );
	},
	...
	_initializeSSDPClients: function _initializeSSDPClients(){
        ...
        adapters.forEach(adapter => {
            ...
            ssdpClient.on('deviceResponse', headers => {
                ...
                var id = this._utilities.md5(headers.LOCATION);
                ...
                var previouslyFoundDevice = previouslyFound.Length > 0 ? previouslyFound[0] : {};

                this._deviceFactory.createDevice(headers.LOCATION, headers, previouslyFoundDevice, 0).then( device => {
