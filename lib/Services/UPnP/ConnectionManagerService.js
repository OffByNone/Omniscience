...
const ConnectionManagerService = Class({
	...
	getCurrentConnectionIds: function getCurrentConnectionIds(device){
		...
		this._soapService.post(this._utilities.getControlUrl(device, this.serviceType),
							   this.serviceType, "GetCurrentConnectionIds"
								  ...
								  var currentConnectionId = this._xmlParser.getText(response.xml,"Envelope Body GetCurrentConnectionIDsResponse ConnectionIDs");
							   ...
							   this.serviceType, "GetCurrentConnectionInfo", { ConnectionID: device.connectionId }
								  ...
								  if(xml)
									  var currentConnectionInfo = {
									  	status: this._xmlParser.getText(xml,"Status"),
									  	direction: this._xmlParser.getText(xml,"Direction"),
									  	peerConnectionId: this._xmlParser.getText(xml,"PeerConnectionID"),
							  			peerConnectionManager: this._xmlParser.getText(xml,"PeerConnectionManager"),
							  			protocolInfo: this._parseProtocolResponse(this._xmlParser.getText(xml,"ProtocolInfo")),
							  			avTransportId: this._xmlParser.getText(xml,"AVTransportID"),
							  			rcsID: this._xmlParser.getText(xml,"RcsID"),
										_raw: response.text
									  };
	...
	getProtocolInfo: function getProtocolInfo(device){
		...
		this._soapService.post(this._utilities.getControlUrl(device, this.serviceType), this.serviceType, "GetProtocolInfo").then( response => {
			var sinkText = this._xmlParser.getText(response.xml,"Envelope Body GetProtocolInfoResponse Sink");
			var sourceText = this._xmlParser.getText(response.xml,"Envelope Body GetProtocolInfoResponse Source");
