...
const ConnectionManagerService = Class({
	...
	getCurrentConnectionIds: function getCurrentConnectionIds(device){
		...
		this._soapService.post(this._utilities.getControlUrl(device, this.serviceType),
							   this.serviceType, "GetCurrentConnectionIds"
								  ...
								  var currentConnectionId = this._xmlParser.getTextFromXml(response.xml,"Envelope Body GetCurrentConnectionIDsResponse ConnectionIDs");
							   ...
							   this.serviceType, "GetCurrentConnectionInfo", { ConnectionID: device.connectionId }
								  ...
								  if(xml)
									  var currentConnectionInfo = {
									  	status: this._xmlParser.getTextFromXml(xml,"Status"),
									  	direction: this._xmlParser.getTextFromXml(xml,"Direction"),
									  	peerConnectionId: this._xmlParser.getTextFromXml(xml,"PeerConnectionID"),
							  			peerConnectionManager: this._xmlParser.getTextFromXml(xml,"PeerConnectionManager"),
							  			protocolInfo: this._parseProtocolResponse(this._xmlParser.getTextFromXml(xml,"ProtocolInfo")),
							  			avTransportId: this._xmlParser.getTextFromXml(xml,"AVTransportID"),
							  			rcsID: this._xmlParser.getTextFromXml(xml,"RcsID"),
										_raw: response.text
									  };
	...
	getProtocolInfo: function getProtocolInfo(device){
		...
		this._soapService.post(this._utilities.getControlUrl(device, this.serviceType), this.serviceType, "GetProtocolInfo").then( response => {
			var sinkText = this._xmlParser.getTextFromXml(response.xml,"Envelope Body GetProtocolInfoResponse Sink");
			var sourceText = this._xmlParser.getTextFromXml(response.xml,"Envelope Body GetProtocolInfoResponse Source");
