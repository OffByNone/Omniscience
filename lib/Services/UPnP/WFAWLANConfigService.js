...
const WFAWLANConfigService = Class({
	...
	initialize: function initialize(emitter, defer, soapService, subscriptionService, DOMParser, utilities) {
		...
		this._serviceType = Constants.ServiceTypes.filter(y=> y[0] === "WFA WLAN Config").map(y=> y[1])[0];
	...
	getDeviceInfo: function getDeviceInfo (device) {
		var deferred = this._defer();
		this._soapService.post(this._utilities.getControlUrl(device, this._serviceType),
                               this._serviceType, "GetDeviceInfo", { }
                              ).then(response => deferred.resolve(response));
		return deferred.promise;
	},
	...
	setSTASettings: function setSTASettings(device){
		var deferred = this._defer();
		this._soapService.post(this._utilities.getControlUrl(device, this._serviceType),
                               this._serviceType, "SetSTASettings", {}
                              ).then(response => deferred.resolve(response));
		return deferred.promise;
	},
	subscribe: function subscribe(device){
		var deferred = this._defer();
		this._subscriptionService.subscribe(device, this._serviceType, 'WFAWLANConfig' ).then( response => deferred.resolve( response ) );
		return deferred.promise;
	},
