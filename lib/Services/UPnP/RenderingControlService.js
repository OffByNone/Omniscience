...
const RenderingControlService = Class({
	...
	initialize: function initialize(emitter, defer, soapService, subscriptionService, DOMParser, utilities, xmlParser) {
		...
		this.serviceType = Constants.ServiceTypes.filter(y=> y[0] === "Rendering Control").map(y=> y[1])[0];
	...
	getMute: function getMute(device){
		...
		this._soapService.post(this._utilities.getControlUrl(device, this.serviceType),
							   this.serviceType, "GetMute", { InstanceID: device.instanceId, Channel: 'Master' }
							  ).then(response => {
							  	var isMuted = this._xmlParser.getTextFromXml(response.xml,"Envelope Body GetMuteResponse CurrentMute") == 1;
	...
	getVolume: function getVolume(device){
		var deferred = this._defer();
		this._soapService.post(this._utilities.getControlUrl(device, this.serviceType),
									  this.serviceType, "GetVolume", { InstanceID: device.instanceId, Channel: 'Master' }
									 ).then(response => {
									 	var volume = this._xmlParser.getTextFromXml(response.xml,"Envelope Body GetVolumeResponse CurrentVolume");
	...
	listPresets: function listPresets(device){
		var deferred = this._defer();
		this._soapService.post(this._utilities.getControlUrl(device, this.serviceType),
							   this.serviceType, "ListPresets", { InstanceID: device.instanceId }
							  ).then(response => {
							  	var presets = this._xmlParser.getTextFromXml(response.xml,"Envelope Body ListPresetsResponse CurrentPresetNameList");
	...
	selectPresets: function selectPresets(device, presetName){
		var deferred = this._defer();
		this._soapService.post(this._utilities.getControlUrl(device, this.serviceType),
							   this.serviceType, "SelectPresets", { InstanceID: device.instanceId, PresetName: presetName }
	...
	setMute: function setMute(device, desiredMute){
		var deferred = this._defer();
		this._soapService.post(this._utilities.getControlUrl(device, this.serviceType),
							   this.serviceType, "SetMute", { InstanceID: device.instanceId, Channel: 'Master', DesiredMute: desiredMute }
							  ).then(response => deferred.resolve(response));
		return deferred.promise;
	},
	setVolume: function setVolume(device, desiredVolume){
		var deferred = this._defer();
		this._soapService.post(this._utilities.getControlUrl(device, this.serviceType),
							   this.serviceType, "SetVolume", { InstanceID: device.instanceId, Channel: 'Master', DesiredVolume: desiredVolume }
							  ).then(response => deferred.resolve(response));
		return deferred.promise;
	},
	subscribe: function subscribe(device){
		var deferred = this._defer();
		this._subscriptionService.subscribe(device, this.serviceType, 'renderingControl').then( response => deferred.resolve( response ) );
		return deferred.promise;
	},
