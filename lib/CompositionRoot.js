...
class CompositionRoot {
    ...
    createDeviceLocator(){
        return this._ipResolver.resolveIPs().then( (ipAddresses) => {
        	return new DeviceLocator(Eventer, MD5, Firefox.timersSDK(), fetch,
                        new DeviceFactory(fetch, new XmlParser(Firefox.createDOMParser()), new UrlProvider(Firefox.urlSDK()), MD5),
                        new ActiveSearcher(Eventer, this.createSSDPClients(Constants.activeEventPrefix, ipAddresses)),
                        new PassiveSearcher(Eventer, this.createSSDPClients(Constants.passiveEventPrefix, ipAddresses, Constants.MulticastPort)),
    ...
    createTab(){
    	return new Tab(this._serviceExecutor, Eventer, Firefox.tabsSDK(), 
					...
					new TCPSender(Eventer, Firefox.timersSDK(), new TCPSocketProvider(Firefox), new SocketSender(),NetworkingUtils));
    }
