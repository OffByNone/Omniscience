...
class CompositionRoot {
    constructor() {
        ...
        this._fxosWebServer = new fxosWebServer(new UrlProvider(UrlSdk));
        ...
        this._deviceServices = new DeviceServices(Emitter, new ServiceInfoFactory(fetch, ComponentFactory.createDOMParser(), new XmlParser(), new UrlProvider(UrlSdk), MD5 ),
									...
									this._fxosWebServer,
									new MatchStickService(Emitter, new MessageFactory(), new MessageService(new TransportServiceFactory(Emitter, ComponentFactory), Emitter, Timer), new UrlProvider(UrlSdk)));
    }
    createDeviceLocator(){
        return this._ipResolver.resolveIPs().then( (ipAddresses) => {
            return new DeviceLocator(Emitter, MD5,
                        new DeviceFactory(ComponentFactory.createDOMParser(), fetch, new MessageFactory(), Emitter, new XmlParser(), new UrlProvider(UrlSdk), MD5),
    ...
    createTab(){
        return new Tab(this._deviceServices, Emitter, tabs, new FilePicker(ComponentFactory.createMimeService(), new FilePickerFactory(ComponentFactory), ComponentFactory.filePickerConstants(), new LocalFileFactory(ComponentFactory), windowUtils), new FileSharer(this._httpd, new UrlProvider(UrlSdk)));
    }
