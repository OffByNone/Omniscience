...
class CompositionRoot {
	constructor(sdk) {
		...
		this._httpServer = new HttpServer(this._sdk.createTCPSocket(), new UrlProvider(this._sdk.urlSDK()), new HttpResponder(NetworkingUtils, new SocketSender()), new HttpRequestParser(NetworkingUtils), this._sdk.timersSDK(),
		...
		this._serviceExecutor = new ServiceExecutor(new ServiceInfoFactory(fetch, new XmlParser(this._sdk.createDOMParser()), new UrlProvider(this._sdk.urlSDK()), MD5 ),
		...
		this._filePicker = new FilePicker(new FileUtils(this._sdk.getMimeService()), new FilePickerFactory(sdk), this._sdk.filePickerConstants(), new LocalFileFactory(sdk), this._sdk.windowUtilsSDK(), PubSub);
		this._fileSharer = new FileSharer(this._httpServer, new UrlProvider(this._sdk.urlSDK()), MD5, PubSub);
		...
		this._tcpSender = new TCPSender(PubSub, this._sdk.timersSDK(), new TCPSocketProvider(sdk), new SocketSender(), NetworkingUtils);
	}
	createDeviceLocator(){
		return this._ipResolver.resolveIPs().then( (ipAddresses) => {
			return new DeviceLocator(PubSub, MD5, this._sdk.timersSDK(), fetch,
                        new DeviceFactory(fetch, new XmlParser(this._sdk.createDOMParser()), new UrlProvider(this._sdk.urlSDK()), MD5),
                        ...
                        this._sdk.storageSDK(), this._serviceExecutor, this._sdk.notificationsSDK());
		}).then( (deviceLocator) => {
	...
	createButton(){
		...
		var menu = null;
		try { //desktop sdk
			buttons = this._sdk.buttonsSDK();
		}
		catch(e){ //sdk for Android
			menu = this._sdk.getNativeWindowMenu();
		}

		return new Button(PubSub, buttons, menu);
	}
	createPanel(){
		return new Panel(PubSub, this._sdk.panelSDK());
	}
	createTab(){
		return new Tab(PubSub, this._sdk.tabsSDK());
	}
