...
class CompositionRoot {
	constructor() {
		...
		this._serviceExecutor = new ServiceExecutor(new ServiceInfoFactory(fetch, new XmlParser(Firefox.createDOMParser()), new UrlProvider(Firefox.urlSDK()), MD5 ),
									new ExecutableServiceFactory(new XmlParser(Firefox.createDOMParser()), new SOAPService(fetch, Firefox.createDOMParser(), Utilities)), PubSub);

		this._filePicker = new FilePicker(new FileUtils(Firefox.getMimeService()), new FilePickerFactory(Firefox), Firefox.filePickerConstants(), new LocalFileFactory(Firefox), Firefox.windowUtilsSDK(), PubSub);
		this._fileSharer = new FileSharer(this._httpServer, new UrlProvider(Firefox.urlSDK()), MD5, PubSub);
		...
		this._tcpSender = new TCPSender(PubSub, Firefox.timersSDK(), new TCPSocketProvider(Firefox), new SocketSender(), NetworkingUtils);
	}
	createDeviceLocator(){
		return this._ipResolver.resolveIPs().then( (ipAddresses) => {
			return new DeviceLocator(PubSub, MD5, Firefox.timersSDK(), fetch,
                        ...
                        new AccessPointSearcher(PubSub, Firefox.getWifiMonitor()),
	...
	createPanel(){
		return new Panel(this._serviceExecutor, PubSub, Firefox.panelSDK());
	}
	createTab(){
		return new Tab(PubSub, Firefox.tabsSDK());
	}
