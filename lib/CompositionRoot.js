...
class CompositionRoot {
	constructor(sdk) {
		...
		this._httpServer = new HttpServer(this._sdk.createTCPSocket(), new UrlProvider(this._sdk.url()), new HttpResponder(NetworkingUtils, new SocketSender()), new HttpRequestParser(NetworkingUtils), this._sdk.timers(),
							new FileResponder(this._sdk.createFile(), new FileUtils(this._sdk.getMimeService()),
												new LocalFileFactory(this._sdk), new HttpResponder(NetworkingUtils, new SocketSender()),
		...
		this._filePicker = new FilePicker(new FileUtils(this._sdk.getMimeService()), new FilePickerFactory(this._sdk), this._sdk.filePickerConstants(), new LocalFileFactory(this._sdk), this._sdk.windowUtils(), PubSub);
		...
		this._tcpSender = new TCPSender(PubSub, this._sdk.timers(), new TCPSocketProvider(this._sdk), new SocketSender(), NetworkingUtils);
	}
	createDeviceLocator(){
		var ipResolver = this._sdk.createIPResolver();

		return ipResolver.resolveIPs().then( (ipAddresses) => {
	...
	createSSDPClients(eventPrefix, ipAddresses, sourcePort){
		return ipAddresses.map((ipAddress) => {
			var ssdpClient = new SSDPClient(eventPrefix, PubSub, Utilities, this._sdk.createUDPSocket(sourcePort), NetworkingUtils);
