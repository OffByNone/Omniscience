...
const DeviceDiscoverer = Class({
	...
	initialize: function initialize(options) {
		...
		this._ssdp.on('deviceResponse', (headers) => {
			...
			if (type !== constants.UniformResourceNames.DIALService) {
				if(type === 'unknown') console.log(headers.ST);
				return;
			}

			DIALDevice.createDevice(headers.LOCATION).then((device) => {
				device.on('additionalInformationFound', () => emit(this, 'additionalInformationFound', device));
				emit(this, 'deviceFound', device);
			});
		});
		this._ssdp.on('error', (error) => console.log(error));
	},
