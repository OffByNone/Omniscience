...
(function () {
    ...
    function executeCommands(commands, type) {
        commands.forEach(function (command) {
            return info.innerHTML += type + " " + command + "<br />";
        });
        commands.forEach(function (command) {
            return media[type][command]();
        });
    }

    function setProperties(properties, type) {
        for (var property in properties) if (properties.hasOwnProperty(property)) media[type][property] = properties[property];

        for (var property in properties) if (properties.hasOwnProperty(property)) info.innerHTML += type + " " + property + " " + properties[property] + "<br />";
    }
